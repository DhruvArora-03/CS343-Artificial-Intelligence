...
def fillObsCPT(bayesNet, gameState):
    ...
    # every possible house position (4 of them)
    for housePos in gameState.getPossibleHouses():
        # all possible observation values (7 of them)
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            # create factor and calculate CPT from each possible assignment
            factor = bn.Factor([obsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            for assignment in factor.getAllPossibleAssignmentDicts():
                prob = -1

                # if obsVar is definetly at food house
                if assignment[FOOD_HOUSE_VAR] == posToVal[housePos]:
                    if assignment[obsVar] == RED_OBS_VAL:
                        ...
                        prob = 0
                # if obsVar is definetly at ghost house
                elif assignment[GHOST_HOUSE_VAR] == posToVal[housePos]:
                    if assignment[obsVar] == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif assignment[obsVar] == BLUE_OBS_VAL:
                        ...
                        prob = 0
                # otherwise are not at a house, so we know obs is none with certainty
                else:
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        left_prob = factor.getProbability(
            {FOOD_HOUSE_VAR: TOP_LEFT_VAL, GHOST_HOUSE_VAR: TOP_RIGHT_VAL, **evidence}
            )
        right_prob = factor.getProbability(
            {FOOD_HOUSE_VAR: TOP_RIGHT_VAL, GHOST_HOUSE_VAR: TOP_LEFT_VAL, **evidence}
            )
        
        leftExpectedValue = WON_GAME_REWARD * left_prob + GHOST_COLLISION_REWARD * (1 - left_prob)
        rightExpectedValue = WON_GAME_REWARD * right_prob + GHOST_COLLISION_REWARD * (1 - right_prob)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        for prob, assignment in self.getExplorationProbsAndOutcomes(evidence):
            new_evidence = evidence.copy()
            new_evidence.update(assignment)
            temp = prob * max(self.computeEnterValues(new_evidence, enterEliminationOrder))
            expectedValue += temp
