...
def enhancedFeatureExtractorDigit(datum):
    ...
    features['has_no_loops'] - there are 0 enclosed regions of blank pixels
    features['has_1_loop'] - there is 1 enclosed region of blank pixels
    features['has_2_loops'] -  there are 2 enclosed regions of blank pixels
    ##
    """
    features =  basicFeatureExtractorDigit(datum)

    "*** YOUR CODE HERE ***"
    # offsets to reach all neighbors of a pixel
    offsets = [(x, y) for x in [-1, 0, 1] for y in [-1, 0 ,1] if x != 0 or y != 0]

    # find num of continuous blank/non-blank sections
    num_blank_spaces = 0
    # classify each coord
    coords_to_check = set([(x, y) for x in range(datum.width) for y in range(datum.height)])
    while len(coords_to_check) > 0:
        value = coords_to_check.pop()
        # start the frontier with some random point
        frontier = set([value])
        is_blank_region = datum.getPixel(*value) == 0
        # increment the proper feature counter
        if is_blank_region:
            num_blank_spaces += 1

        # find the entire continuous regioum
        while len(frontier) > 0:
            curr = frontier.pop()
            # try all neighbors
            for offset in offsets:
                # apply offset
                candidate = (curr[0] + offset[0], curr[1] + offset[1])

                # check if candidate still needs to be classified
                if candidate not in coords_to_check:
                    continue

                # check if candidate matches the current region
                if (datum.getPixel(*candidate) == 0) == is_blank_region:
                    # update sets accordingly
                    coords_to_check.remove(candidate)
                    frontier.add(candidate)

    features['has_no_loops'] = 1 if num_blank_spaces == 1 else 0
    features['has_1_loop'] = 1 if num_blank_spaces == 2 else 0
    features['has_2_loops'] = 1 if num_blank_spaces == 3 else 0
...
def enhancedPacmanFeatures(state, action):
    ...
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    # offsets to reach all neighbors of a food
    offsets = [(x, y) for x in [-1, 0, 1] for y in [-1, 0 ,1] if x != 0 or y != 0]

    successor: GameState = state.generateSuccessor(0, action)
    ...
    # print(state_food)
    # food_dists = [mazeDistance(pac_pos, food, successor) for food in state_food]
    # features['dist_to_closest_food'] = min(food_dists)
    
    # search for food
    queue = util.PriorityQueue() # structure is ((x, y), dist)
    visited = set()
    queue.push((pac_pos, 0), 0)

    food_left = 4
    looking_for_food = True
    capsules_left = len(capsules)
    looking_for_capsule = len(capsules) > 0
    features['min_dist_to_capsule'] = 0
    ghosts_left = len(ghosts)

    ghost_dists = sorted([util.manhattanDistance(pac_pos, g) for g in ghosts])
    for i in range(2):
        features['min_ghost_%d' % i] = ghost_dists[i]

    while not queue.isEmpty() and (looking_for_food or looking_for_capsule or ghosts_left > 0):
        loc, dist = queue.pop()
        ...
        if looking_for_food and state_food[loc[0]][loc[1]]:
            if dist == 0:
                dist = .1
            features['min_dist_to_food_%d' % food_left] = 100/dist
            if loc == pac_pos:
                features['min_manhattan_dist_to_food_%d' % food_left] = 100/(util.manhattanDistance(loc, pac_pos) + .1)
            else:
                features['min_manhattan_dist_to_food_%d' % food_left] = 100/util.manhattanDistance(loc, pac_pos)
            food_left -= 1
            if food_left < 1:
                looking_for_food = False
        
        # check if reached capsule
        if looking_for_capsule and loc in capsules:
            if dist == 0:
                dist = .1
            features['min_dist_to_capsule_%d' % capsules_left] = 10/dist
            capsules_left -= 1
            if capsules_left < 1:
                looking_for_capsule = False

        # check if found ghost
        if ghosts_left > 0 and loc in ghosts:
            features['ghost_%d' % ghosts_left] = dist
            ghosts_left -= 1
        ...
        for offset in offsets:
            ...
            if walls[candidate[0]][candidate[1]] or candidate in visited:
                continue
            
            queue.push((candidate, dist + 1), min([util.manhattanDistance(pac_pos, g) for g in ghosts]))
